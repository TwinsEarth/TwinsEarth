...
### V1.0.1 — 周期 Agent（2026-09-23）

新增四层嵌套周期 Agent 框架（`cycle_agents/`），将人类历史动力学建模为四级嵌套的状态机：

- **零级祖周期** `ClimateAgent`：米兰科维奇三周期（偏心率10万年/倾角4.1万年/岁差2.3万年）+ CO2 温室效应 + 生态承载力
- **一级母周期** `PopulationAgent`：逻辑斯蒂增长 + 崩溃阈值 + 年龄结构演化 + 技术乘数影响承载力
- **二级子周期** `InstitutionAgent`：制度熵增 + 土地兼并 + 财政健康度 + 改革窗口 + 社会危机等级
- **三级孙周期** `EventAgent`：商业周期叠加 + 随机冲击 + 市场情绪 + 社会动荡
- **双向反馈** `NestedCycleSystem`：自上而下压力传导（气候→人口→制度→事件）+ 自下而上扰动反馈（事件→制度→人口→气候）
